...
abstract class ResolveProcessor<ResolveResult>(private val place: PsiElement) : PsiScopeProcessor {
    ...
    protected val PsiElement.hasNoError
        get() = (this as? StubBasedPsiElement<*>)?.stub != null || !PsiTreeUtil.hasErrorElements(
            this
        )
    ...
    fun isInScope(element: PsiElement) = when {
        ...
        element.isParameter -> PsiTreeUtil.isAncestor(
            PsiTreeUtil.getParentOfType(
                element, ZigFnDeclaration::class.java
            ), place, true
        )
        element.isFunctionName -> PsiTreeUtil.isAncestor(
            PsiTreeUtil.getParentOfType(
                element, ZigFnDeclaration::class.java
            )?.parent, place, true
        )
